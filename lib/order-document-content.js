...
function drawPaymentDetails(state) {
    ...
    const boxHeight = sideBySide ? Math.max(totalsBox.boxHeight, 118) : 118;
    ...
    const lines = [
        `IBAN : ${iban}`,
        `Titulaire du compte : ${accountHolder}`,
        `Banque : ${bankName}`,
        `Montant : ${formatMoney(order.amount_cents, order.currency)}`,
    ...
    pdf.text("Informations de paiement", boxX + 18, topY - 24, {
        font: "F2",
        size: 12,
    });
