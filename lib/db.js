...
const DEFAULT_SETTINGS = {
    ...
    shipping_note: "Remise en main propre ou expédition à définir selon le produit.",
    bank_account_holder: "",
    bank_name: "",
    bank_account_number: "",
    bank_iban: "",
...
function parseValidConfigurations(value, optionGroups) {
    ...
    return uniqueStrings(parseLineList(value))
        ...
        .filter((configuration, index, configurations) =>
            configurations.findIndex((item) => JSON.stringify(item) === JSON.stringify(configuration)) === index
        );
}
...
function parseProduct(product) {
    ...
    const valid_configurations = parseJsonArray(product.valid_configurations_json)
        .map((configuration) => (Array.isArray(configuration) ? configuration : []))
        ...
        .filter((configuration) => configuration.length === option_groups.length)
        .filter((configuration) => configuration.every((selection, index) =>
...
function listPublishedProducts(db) {
    return db.prepare(`
        SELECT *
        FROM products
        WHERE published = 1
...
function updateOrderRecord(db, orderId, updates = {}) {
    ...
    db.prepare(`
        ...
        SET status = ?,
            ...
            updated_at = ?
        WHERE id = ?
    `).run(nextStatus, JSON.stringify(nextMetadata), nowIso(), orderId);
    ...
    const transaction = db.transaction(() => {
        db.prepare(`
            ...
            SET status = 'paid',
                ...
                updated_at = ?
            WHERE id = ?
        `).run(JSON.stringify(nextMetadata), nowIso(), orderId);
        ...
        for (const item of order.items) {
            updateInventory.run(item.quantity, nowIso(), item.product_id);
        }
    });
