...
function normalizeProductInput(input) {
    ...
    const optionGroups = parseOptionGroupsStrict(input.option_groups);
    const infoRows = parseInfoRows(input.info_rows);
    const validConfigurations = parseValidConfigurationsStrict(input.valid_configurations, optionGroups);
...
function getDashboardStats(db) {
    ...
    const potentialRevenueCents = listAdminProducts(db).reduce((total, product) => {
    ...
    const transaction = db.transaction(() => {
        ...
        if (!paymentAlreadyRecorded) {
            for (const item of nextItems) {
                let product = productsToUpdate.get(item.product_id);
                if (!product) {
                    const record = getProductById(db, item.product_id);
                    if (!record) {
                        continue;
                    }
                    ...
                    productsToUpdate.set(item.product_id, product);
                }
                ...
                if (item.quantity > availableQuantity) {
                    throw new Error(`Stock insuffisant pour ${item.name}. La commande ne peut pas être finalisée.`);
                }

                const inventoryUpdate = updateInventory.run(item.quantity, timestamp, item.product_id, item.quantity);
                ...
                if (Number.isInteger(configuration.quantity)) {
                    configuration.quantity = Math.max(configuration.quantity - item.quantity, 0);
                }

                if (Array.isArray(configuration.service_tags) && configuration.service_tags.length) {
                    const requestedServiceTags = Array.isArray(item.service_tags)
                        ? uniqueStrings(item.service_tags.map((tag) => String(tag || "").trim()))
                        : [];
                    const unavailableRequestedTags = requestedServiceTags.filter((tag) => !configuration.service_tags.includes(tag));
                    ...
                    const requestedTagSet = new Set(requestedServiceTags);
                    const remainingConfigurationTags = configuration.service_tags.filter((tag) => !requestedTagSet.has(tag));
                    const missingCount = Math.max(item.quantity - requestedServiceTags.length, 0);
                    const autoAssignedTags = missingCount > 0
                        ? remainingConfigurationTags.slice(0, missingCount)
                        : [];
                    const consumedTags = [...requestedServiceTags, ...autoAssignedTags].slice(0, item.quantity);
                    const consumedTagSet = new Set(consumedTags);

                    item.service_tags = consumedTags;
