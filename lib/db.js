...
function canDeleteTestOrder(order) {
    ...
    return Boolean(order.metadata?.inventory_reserved_at);
}
...
function deleteTestOrder(db, orderId) {
    return db.transaction(() => {
        ...
        if (!canDeleteTestOrder(order)) {
            throw new Error("Seules les commandes de test hors ligne avec un historique de réservation peuvent être supprimées ainsi.");
        }
