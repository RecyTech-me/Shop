...
function createDomainServices({
    ...
    let reservationCleanupInitialTimer = null;
    ...
    if (backgroundTasksEnabled && config.paymentReservations.cleanupIntervalMs > 0) {
        reservationCleanupInitialTimer = setTimeout(runReservationCleanup, 0);
        reservationCleanupInitialTimer.unref?.();
        reservationCleanupInterval = setInterval(runReservationCleanup, config.paymentReservations.cleanupIntervalMs);
    ...
    return {
        ...
        stop() {
            if (reservationCleanupInitialTimer) {
                clearTimeout(reservationCleanupInitialTimer);
            }
