...
const DEFAULT_SETTINGS = {
    ...
    tagline: "Matériel informatique remis en état, avec garantie et stock affiché.",
    hero_title: "Du matériel reconditionné, prêt à servir.",
    hero_text: "Des appareils remis en état, et Linux préinstallé sur les ordinateurs compatibles, à prix accessibles.",
...
function runSchemaMigrations(db) {
    ...
    runSchemaMigration(db, "2026-07-21-order-idempotency", () => {
        ensureColumn(db, "orders", "idempotency_key", "TEXT");
    });
    runSchemaMigration(db, "2026-07-21-admin-auth-version", () => {
        ensureColumn(db, "admins", "auth_version", "INTEGER NOT NULL DEFAULT 0");
    });
    runSchemaMigration(db, "2026-07-21-unique-provider-references", () => {
        const duplicate = db.prepare(`
            SELECT provider, COUNT(*) AS reference_count
            FROM orders
        ...
        `).get();
        if (duplicate) {
            throw new Error(`Cannot enforce unique provider references: duplicate ${duplicate.provider} references exist`);
        }

        db.exec(`
            CREATE UNIQUE INDEX IF NOT EXISTS idx_orders_unique_provider_reference
            ON orders (provider, provider_reference)
            WHERE provider_reference IS NOT NULL;

            DROP INDEX IF EXISTS idx_orders_provider_reference;
        `);
    });
    runSchemaMigration(db, "2026-07-22-catalog-consistency", () => {
        backfillProductPriceRanges(db);
        repairCatalogConsistency(db);
    });
}
...
function seedAdmin(db, env) {
    ...
    const username = String(env.ADMIN_USERNAME || "").trim() || "admin";
...
function replaceJsonText(value, replacements) {
    let parsed;
    try {
    ...
    function visit(current) {
        if (typeof current === "string") {
            return replaceAllText(current, replacements);
        }
...
function repairProductContent(db, slug, replacements) {
    const product = db.prepare(`
        SELECT id, name, short_description, description, option_groups_json,
    ...
    const next = {
        name: replaceAllText(product.name, replacements),
        short_description: replaceAllText(product.short_description, replacements),
        description: replaceAllText(product.description, replacements),
        option_groups_json: replaceJsonText(product.option_groups_json, replacements),
        info_rows_json: replaceJsonText(product.info_rows_json, replacements),
        valid_configurations_json: replaceJsonText(product.valid_configurations_json, replacements),
    };
    if (Object.entries(next).every(([key, value]) => value === product[key])) {
        return;
    }
    ...
    db.prepare(`
        ...
        WHERE id = @id
    `).run({ ...next, id: product.id, updated_at: nowIso() });
}

function renameProductSlug(db, previousSlug, canonicalSlug) {
    const product = db.prepare("SELECT id FROM products WHERE slug = ?").get(previousSlug);
    if (!product) {
        return;
    }

    const collision = db.prepare("SELECT id FROM products WHERE slug = ? AND id != ?").get(canonicalSlug, product.id);
    if (collision) {
        throw new Error(`Cannot canonicalize product slug ${previousSlug}: ${canonicalSlug} already exists`);
    }

    db.prepare("UPDATE products SET slug = ?, updated_at = ? WHERE id = ?")
        .run(canonicalSlug, nowIso(), product.id);
}

function repairCatalogConsistency(db) {
    renameProductSlug(db, "epson-eb-99ou", "epson-eb-990u");

    repairProductContent(db, "epson-eb-990u", [
        ["Epson EB-99OU", "Epson EB-990U"],
        ["en parfait état", "en très bon état"],
        ["parfait état", "très bon état"],
        ["Parfait état", "Très bon état"],
    ]);
    repairProductContent(db, "dell-latitude-5580", [
        ["HDD ou SDD 256 Go", "HDD ou SSD 256 Go selon variante"],
        ["Excellente ou mauvaise", "Très bonne ou faible selon variante"],
        ["Excellente", "Très bonne"],
        ["Mauvaise", "Faible"],
    ]);
    repairProductContent(db, "dell-p2016", [
        ["Parfait (10/10)", "Très bon"],
    ]);
    repairProductContent(db, "dell-p2412h", [
        ["État=Parfait", "État=Bon"],
        ["Parfait", "Bon"],
    ]);
    repairProductContent(db, "dell-p2417h", [
        ["Fonctionne parfaitement, état 10/10", "Testé, entièrement fonctionnel et en très bon état"],
        ["Parfait état (10/10)", "Très bon état, testé et entièrement fonctionnel"],
    ]);
    repairProductContent(db, "epson-eb-1980wu", [
        ["qualité 10/10", "très bon état"],
        ["Qualité du beamer", "État"],
        ["10/10", "Très bon état, testé et entièrement fonctionnel"],
    ]);
    repairProductContent(db, "epson-585w", [
        ["en parfait état", "en très bon état"],
        ["parfait état", "très bon état"],
    ]);
    repairProductContent(db, "dell-latitude-e5570", [
        ["Dell Latitude 5570", "Dell Latitude E5570"],
    ]);
    repairProductContent(db, "clavier-dell-y-u0003-del", [
        ["Selon arrivage", "Variable selon l’exemplaire — contactez RecyTech avant de commander pour confirmer la disposition disponible"],
    ]);

    const replaceSetting = db.prepare(`
        UPDATE settings
        SET value = ?
        WHERE key = ? AND value = ?
    `);
    replaceSetting.run(DEFAULT_SETTINGS.hero_text, "hero_text", "Transformer les vieux PC en nouvelles opportunités.");
    replaceSetting.run(
        DEFAULT_SETTINGS.tagline,
        "tagline",
        "Ordinateurs reconditionnés, Linux préinstallé, impact local."
    );
    replaceSetting.run(
        DEFAULT_SETTINGS.hero_points,
        "hero_points",
        [
...
function initializeDatabase(databasePath, env) {
    ...
    try {
        ...
        try {
            ...
            logger.warn(`[database] Failed to close database after initialization error: ${closeError.message}`);
        }
        throw error;
    }
}
