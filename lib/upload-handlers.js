...
const PRODUCT_DERIVATIVE_WIDTH = 1600;
const SETTINGS_DERIVATIVE_WIDTH = 1920;
...
function cleanupUploadedFiles(files = []) {
    for (const file of files) {
        const paths = [
            file?.path,
            ...(Array.isArray(file?.generatedPaths) ? file.generatedPaths : []),
        ].filter(Boolean);
...
function validateStoredImageUploads(files = []) {
    ...
    const invalidFiles = files.filter((file) => {
        const detectedFormat = detectStoredImageFormat(file.path);
        const declaredFormat = formatByMimeType[file.mimetype];
        return !detectedFormat || (declaredFormat && detectedFormat !== declaredFormat);
    });
...
async function generateUploadDerivative(file, { width }) {
    if (!file?.path || detectStoredImageFormat(file.path) === "gif") {
        return file;
    }

    const parsed = path.parse(file.filename);
    const derivativeFilename = `${parsed.name}-display.webp`;
    const derivativePath = path.join(path.dirname(file.path), derivativeFilename);
    ...
    try {
        ...
        file.optimizedFilename = derivativeFilename;
        file.optimizedPath = derivativePath;
        file.generatedPaths = [...(file.generatedPaths || []), derivativePath];
    } catch (error) {
        logger.warn(`[uploads] Image derivative generation failed for ${file.filename}: ${error.message}`);
    }

    return file;
}

async function generateUploadDerivatives(files = [], options) {
    for (const file of files) {
        await generateUploadDerivative(file, options);
    }
}

function productUploadFiles(req) {
    return [
...
function uploadUrl(file, folder) {
    ...
    return `/static/uploads/${folder}/${file.optimizedFilename || file.filename}`;
}
...
function createUploadHandlers(options) {
    ...
    function uploadRedirectTarget(req, fallback) {
        return getSafeRedirectTarget(req.get("referer") || req.originalUrl, fallback);
    }
    ...
    function ensureUploadDirectory(req, res, directoryPath) {
        try {
            ...
            saveSessionAndRedirect(req, res, uploadRedirectTarget(req, "/admin"));
            return false;
        }
    }
    ...
    function withProductUploads(req, res, next) {
        ...
        productImageUpload.fields([
            ...
            if (error) {
                ...
                return saveSessionAndRedirect(req, res, uploadRedirectTarget(req, "/admin/products/new"));
            }

            try {
                validateStoredImageUploads(productUploadFiles(req));
            ...
            generateUploadDerivatives(productUploadFiles(req), { width: PRODUCT_DERIVATIVE_WIDTH })
                .then(() => {
                ...
                .catch((derivativeError) => {
                    cleanupProductUploads(req);
                    setFlash(req, "error", derivativeError.message || "La préparation des images a échoué.");
                    saveSessionAndRedirect(req, res, uploadRedirectTarget(req, "/admin/products/new"));
                });
        });
    }

    function cleanupProductUploads(req) {
        cleanupUploadedFiles(productUploadFiles(req));
    }

    function cleanupSettingsUpload(req) {
    ...
    function withSettingsUpload(req, res, next) {
        ...
        settingsImageUpload.single("hero_image_file")(req, res, (error) => {
            if (error) {
                ...
                return saveSessionAndRedirect(req, res, uploadRedirectTarget(req, "/admin/settings"));
            }
            ...
            generateUploadDerivatives(req.file ? [req.file] : [], { width: SETTINGS_DERIVATIVE_WIDTH })
                .then(() => {
                ...
                .catch((derivativeError) => {
                    cleanupSettingsUpload(req);
                    setFlash(req, "error", derivativeError.message || "La préparation de l'image a échoué.");
                    saveSessionAndRedirect(req, res, uploadRedirectTarget(req, "/admin/settings"));
                });
        });
    }
