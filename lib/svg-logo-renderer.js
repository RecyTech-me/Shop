...
const PDF_LOGO_LIGHT_COLOR = "#FFFFFF";
const SOURCE_LOGO_LIGHT_COLOR = "#E2E2E0";
...
function getSvgPaint(value) {
    ...
    if (!color || color === "none" || !/^#[0-9a-f]{3,8}$/i.test(color)) {
        return null;
    }

    const normalizedColor = color.slice(0, 7).toUpperCase();
    return normalizedColor === SOURCE_LOGO_LIGHT_COLOR
        ? PDF_LOGO_LIGHT_COLOR
        : normalizedColor;
}
