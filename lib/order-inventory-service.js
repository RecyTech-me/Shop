...
function canReleaseInventoryReservation(metadata = {}, options = {}) {
    return isInventoryReserved(metadata)
        && (options.allowRecordedPayment === true || !metadata.payment_recorded_at);
}
...
function createInventoryMutator({ db, getProductById, timestamp, strictProductRestore = false }) {
    ...
    function consumeProductQuantity(productId, itemName, quantity, selectedOptions = [], requestedServiceTags = []) {
        ...
        if (!product) {
            throw new Error(`Produit introuvable pour ${itemName}. La commande ne peut pas être finalisée.`);
        }
    ...
    function restoreProductQuantity(productId, quantity, selectedOptions = [], serviceTags = []) {
        const product = getMutableProduct(productId);
        if (!product) {
            if (strictProductRestore) {
                throw new Error("Un produit de la commande n'existe plus. Le stock ne peut pas être restauré automatiquement.");
            }
            return;
        }
...
function restoreOrderInventorySnapshot(db, order, options = {}) {
    const {
        getProductById,
        timestamp = defaultNowIso(),
        strictProductRestore = false,
    } = options;
    const nextItems = cloneOrderItems(order?.items || []);
    const mutator = createInventoryMutator({
        db,
        getProductById,
        timestamp,
        strictProductRestore,
    });
...
function reserveOrderInventory(db, orderId, options = {}) {
    ...
    try {
        ...
        if (inventoryChanged) {
            logger.info(`[inventory] Reserved stock for order ${orderNumber}`);
        }
    } catch (error) {
        logger.error(`[inventory] Reservation failed for order ${orderNumber || orderId}: ${error.message}`);
        throw error;
    }
...
function releaseOrderInventory(db, orderId, options = {}) {
    ...
    const transaction = db.transaction(() => {
        ...
        if (!canReleaseInventoryReservation(order.metadata, { allowRecordedPayment })) {
        ...
        const nextItems = restoreOrderInventorySnapshot(db, order, {
            getProductById,
            timestamp,
            strictProductRestore,
        });
    ...
    try {
        transaction.immediate();
        if (inventoryChanged) {
            logger.info(`[inventory] Released reserved stock for order ${orderNumber}`);
        }
    } catch (error) {
        logger.error(`[inventory] Reservation release failed for order ${orderNumber || orderId}: ${error.message}`);
        throw error;
    }
