...
const PROMO_CODE_COLUMNS = `
    id, code, description, discount_type, discount_value, minimum_order_cents,
    max_redemptions, times_redeemed, starts_on, expires_on, active, created_at, updated_at
`;
...
function getPromoCodeById(db, promoCodeId) {
    return parsePromoCode(db.prepare(`SELECT ${PROMO_CODE_COLUMNS} FROM promo_codes WHERE id = ?`).get(promoCodeId));
}

function getPromoCodeByCode(db, code) {
    return parsePromoCode(
        db.prepare(`SELECT ${PROMO_CODE_COLUMNS} FROM promo_codes WHERE code = ?`).get(normalizePromoCodeValue(code))
    );
}
