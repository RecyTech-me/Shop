...
const SITE_REVIEW_COLUMNS = `
    id, rating, reviewer_name, reviewer_email, title, body, approved, created_at, updated_at
`;
const PUBLIC_SITE_REVIEW_COLUMNS = `
    id, rating, reviewer_name, title, body, approved, created_at, updated_at
`;
...
function listApprovedSiteReviews(db) {
    return db.prepare(`
        SELECT ${PUBLIC_SITE_REVIEW_COLUMNS}
        FROM site_reviews
        WHERE approved = 1
...
function createSiteReview(db, input) {
    ...
    return parseSiteReview(db.prepare(`SELECT ${SITE_REVIEW_COLUMNS} FROM site_reviews WHERE id = ?`).get(result.lastInsertRowid));
}
...
function approveSiteReview(db, reviewId) {
    ...
    return parseSiteReview(db.prepare(`SELECT ${SITE_REVIEW_COLUMNS} FROM site_reviews WHERE id = ?`).get(reviewId));
}
