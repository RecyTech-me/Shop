...
function hydratePublishedProductRows(db, rows) {
    return rows.map(parseProduct).map((product) => hydrateProduct(db, product));
}
...
function listPublishedProductsWithHydratedAvailability(db, sql, values, filters, limit, offset) {
    ...
    while (matches.length < targetCount) {
        ...
        matches.push(...hydratePublishedProductRows(db, rows)
