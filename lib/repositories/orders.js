...
const ORDER_COLUMNS = `
    id, order_number, provider, provider_reference, idempotency_key, status,
...
function createOrder(db, input) {
    const orderNumber = `RCT-${crypto.randomBytes(8).toString("hex").toUpperCase()}`;
    const timestamp = nowIso();
    const createdAt = input.created_at || timestamp;

    const result = db.prepare(`
        INSERT INTO orders (
            order_number, provider, provider_reference, idempotency_key, status,
            customer_name, customer_email, amount_cents, currency,
            items_json, metadata_json, created_at, updated_at
        )
        VALUES (
...
function getOrderByIdempotencyKey(db, idempotencyKey) {
    return parseOrder(
        db.prepare(`SELECT ${ORDER_COLUMNS} FROM orders WHERE idempotency_key = ?`)
            .get(idempotencyKey)
    );
}
...
function updateOrderProviderReference(db, orderId, providerReference, metadata = null) {
    ...
    if (current.provider_reference && current.provider_reference !== providerReference) {
        throw new Error("La référence de paiement de cette commande est déjà définie.");
    }
    ...
    `).run(providerReference, JSON.stringify(nextMetadata), nowIso(), orderId, providerReference);

    if (!result.changes) {
        const latest = getOrderById(db, orderId);
        if (latest?.provider_reference !== providerReference) {
            throw new Error("La référence de paiement de cette commande a été modifiée simultanément.");
        }
    }
